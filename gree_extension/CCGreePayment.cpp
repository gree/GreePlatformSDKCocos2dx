...
CCGreePaymentItem::CCGreePaymentItem(jobject obj){
	mPaymentItemObj = obj;
}
...
CCGreePaymentItem* CCGreePaymentItem::create(const char *itemId, const char* itemName, double unitPrice, int quantity){
	...
	if(obj != NULL){
		item = new CCGreePaymentItem(obj);
		item->autorelease();
	}
	return item;
}
...
jobject CCGreePaymentItem::getPaymentItemObject(){
...
void CCGreePaymentItem::setImageUrl(const char *url){
	if(mPaymentItemObj != NULL){
		setImageUrlJni(mPaymentItemObj, url);
	}
}

void CCGreePaymentItem::setDescription(const char *desc){
	if(mPaymentItemObj != NULL){
		setDescriptionJni(mPaymentItemObj, desc);
	}
}
...
double CCGreePaymentItem::getUnitPrice(){
	if(mPaymentItemObj != NULL){
		return getUnitPriceJni(mPaymentItemObj);
	}
	return -1;
}
...
// CCGreePayment
CCGreePayment::CCGreePayment(jobject obj){
	mPaymentObj = obj;
}
...
void CCGreePayment::request(){
	...
	if(mPaymentObj != NULL){
		setPaymentHandlerJni(mPaymentObj, (void *)this);
		requestWithHandlerJni(mPaymentObj, (void *)this);
	}
}

void CCGreePayment::setCallbackUrl(const char *url){
	if(mPaymentObj != NULL){
		setCallbackUrlJni(mPaymentObj, url);
	}
}
