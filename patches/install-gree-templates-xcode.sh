...
echo 'cocos2d-x template installer including gree-extension'
...
copy_gree_extension_files(){
	echo ...copying gree extension files
	copy_files gree_extension "$LIBS_DIR"
}
...
copy_xcode4_project_templates(){
	...
	print_template_banner "Installing Xcode 4 cocos2d-x gree_extension iOS template"

	DST_DIR="$TEMPLATE_DIR"

	LIBS_DIR="$DST_DIR""lib_gree_extension.xctemplate/libs/"
	mkdir -p "$LIBS_DIR"
	copy_gree_extension_files
